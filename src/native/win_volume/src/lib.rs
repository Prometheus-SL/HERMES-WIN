...
fn set_master_volume_scalar(level: f32) -> bool {
    ...
    let r = with_com(|| -> windows::core::Result<bool> {
        let device_enumerator: IMMDeviceEnumerator =
            unsafe { CoCreateInstance(&MMDeviceEnumerator, None, CLSCTX_ALL)? }?.cast()?;
        ...
        unsafe {
            aev.SetMasterVolumeLevelScalar(level, std::ptr::null_mut())?;
        }
        Ok(true)
    });
    r.is_ok()
...
fn set_mute(mute: bool) -> bool {
    let r = with_com(|| -> windows::core::Result<bool> {
        ...
        let aev: IAudioEndpointVolume = unsafe { endpoint.cast()? };
        unsafe {
            aev.SetMute(if mute { 1 } else { 0 }, std::ptr::null_mut())?;
        }
        Ok(true)
    });
    r.is_ok()
...
fn get_master_volume_scalar() -> f32 {
    let r = with_com(|| -> windows::core::Result<f32> {
        ...
        let mut val: f32 = 0.0;
        unsafe {
            aev.GetMasterVolumeLevelScalar(&mut val)?;
        }
        Ok(val)
    });
    r.unwrap_or(0.0)
